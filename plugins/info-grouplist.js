...
try {    
  ...
  for (let i = 0; i < groups.length; i++) {
    ...
    txt += `*◉ Group ${i + 1}*
    *𓆈 Number:* ${await conn.getName(jid)}
    *𓆈 ID:* ${jid}
    *𓆈 Admin:* ${isBotAdmin ? '✔ yes' : '❌ No'}
    *𓆈 State:* ${participantStatus}
    *𓆈 Total Participants:* ${totalParticipants}
    *𓆈 Link:* ${isBotAdmin ? `https://chat.whatsapp.com/${await conn.groupInviteCode(jid) || '--- (Error) ---'}` : '--- (No admin) ---'}\n\n`;
  }
